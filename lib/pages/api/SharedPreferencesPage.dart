...
class SharedPreferencesClass extends StatefulWidget {
  SharedPreferencesClass({Key? key, required this.title, this.arguments}) : super(key: key);
  final String title;
  dynamic arguments;
  @override
      ...
      body: Container(
        ...
        child: Column(
          children: [
            Container(
              padding: const EdgeInsets.all(10),
              child: const Text('Supported data types are int, double, bool, String and List<String>', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            ),
            Expanded(
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    Input(key: inputKey, value: userName, placeholder: '请输入名字', onChange: (val) {
                      value = val;
                    }),
                    Container(
                      padding: const EdgeInsets.all(10),
                      child: Button(text: '设置名字', onTap: () async {
                    ...
                    Row(
                      ...
                      children: [
                        Button(text: '获取名字', round: true, onTap: () async {
                          final SharedPreferences prefs = await _prefs;
                          setState((){
                            userName = prefs.getString('userName') ?? '';
                          });
                        }),
                        Button(text: '移除名字', round: true, type: 'plain', onTap: () async {
                          final SharedPreferences prefs = await _prefs;
