...
class SliversPage extends StatefulWidget {
  SliversPage({Key? key, required this.title, this.arguments})
      : super(key: key);
  String title;
  dynamic arguments;
  @override
    ...
    return Scaffold(
      ...
      body: CustomScrollView(// 创建一个视窗
        ...
        SliverAppBar(
          ...
          pinned:
              true, // 一直显示在顶部，无视滑动，这样就和普通的导航栏差不多了。区别就是在滑动的时候 SliveAppbar 的底部会有一点点影子
        ...
        SliverPersistentHeader(
          ...
          delegate: _SliverHeaderDelegate(
              '固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题固定标题RR'),
        ),
        SliverList(
            // 类似于 listview
            delegate:
...
class _SliverHeaderDelegate extends SliverPersistentHeaderDelegate {
  ...
  bool shouldRebuild(covariant _SliverHeaderDelegate oldDelegate) {
    // 记得变量为 _SliverHeaderDelegate 而不是 SliverPersistentHeaderDelegate
